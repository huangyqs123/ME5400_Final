...
class TrackerNode:
    def __init__(self):
        ...
        # 订阅图像话题
        self.image_sub = message_filters.Subscriber('/tb3_0/camera/rgb/image_raw', Image)
        # 添加深度图像的订阅者
        self.depth_sub = message_filters.Subscriber('/tb3_0/camera/depth/image_raw', Image)
        self.depth_image = None
        # 使用 ApproximateTimeSynchronizer 同步消息
        self.ts = message_filters.TimeSynchronizer([self.image_sub, self.depth_sub], 10000)
        self.ts.registerCallback(self.synced_images_callback)
    ...
    def synced_images_callback(self, rgb_data, depth_data):
        ...
        if self.is_initialized:
            ...
            # 这里实现追踪逻辑
            rospy.loginfo("Tracking_1")
            _item = self.gTracker.track(cv_image)
            rospy.loginfo("Tracking_2")
            cv2.imshow("track result", _item.getFrame())

            rospy.loginfo("Tracking_3")
            ...
            if lost == False:
                rospy.loginfo("Object is not lost")
                #tracking coordinate
                box = _item.getROI()
                x, y, w, h = box
                # # 使用 ROI 的坐标来获取深度值
                ...
                if roi_depth.size > 0:
                    ...
                    mean_depth = np.nan  # 或者你可以设置一个默认值

                # 如果mean_depth不是nan，那么进行后续处理
