...
class Tracker(object):
    def __init__(self,draw_coord=True):
        ...
        rospy.loginfo("CUDA is available: {0}".format(torch.cuda.is_available()))
        device = torch.device('cuda' if cfg.CUDA else 'cpu')
        rospy.loginfo("Using device: {0}".format(device))
...
class TrackerNode:
    def __init__(self):
        ...
        # 订阅图像话题
        self.image_sub = rospy.Subscriber('/tb3_0/camera/rgb/image_raw', Image, self.image_callback, queue_size=1, buff_size=2**24)
        # 添加深度图像的订阅者
        self.depth_sub = rospy.Subscriber('/tb3_0/camera/depth/image_raw', Image, self.depth_callback, queue_size=1, buff_size=2**24)
